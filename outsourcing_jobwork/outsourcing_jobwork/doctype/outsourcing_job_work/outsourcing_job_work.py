...
class OutsourcingJobWork(Document):
		...
		for d in self.get("outsourcing_job_work"):
			...
			for i in OJWD:
				self.append("outsource_job_work_details",{
							...
							# 'is_finished_item': i.is_finished_item,
						},),
	...
	# @frappe.whitelist()
	# def set_finished_item(self):
	# 	pass
		# frappe.msgprint("hi......!")
		# for fi in self.get('outsource_job_work_details'):
		# 	if fi.is_finished_item:
		# 		if not fi.finished_item:
		# 			fi.finished_item = fi.item_code
		# 	else:
		# 		fi.finished_item = ''
	...
	# 	moc = self.get('outsource_job_work_details' , filters={'finished_item': ['!=', None]})
	# 	if len(moc) == 0:
	# 		frappe.throw('There must be atleast 1 Finished Item Present in field "Finished Item" of Table "Outsource Job Work Details"')  
	# 	elif len(moc) != 1:
	# 		frappe.throw('Multiple items cannot be present in field "Finished Item" of Table "Outsource Job Work Details" ')
	...
	def validate_ojwd(self):
		...
		# moc_item_codes = {entry['item_code'] for entry in moc}
		# # poc_item_codes = poc.item_code()  # Replace with the actual method or property

		# # Validate that all moc.item_code are present in poc.item_code
		# if not all(item_code in poc for item_code in moc_item_codes):
		# 	frappe.throw("Item Code is not matches with Item code from 'Outsourcing BOM'")

		# # Validate that all poc.item_code are present in moc.item_code
		# if not all(item_code in moc_item_codes for item_code in poc):
		# 	frappe.throw("Item Code is not matches with Item code from 'Outsourcing BOM'")
